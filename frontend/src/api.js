...
export async function generatePassword(length) {
    try {
        const res = await fetch(`http://localhost:4467/users/generate-password/${length}`);
...
export async function validationEmail(email) {
    try {
        const res = await fetch("http://localhost:4467/users/verify-email", {
            method: "POST",
...
export async function verifyUser(user) {
    ...
    const serverResponse = await fetch(`${URL}/users/login`, {
        method: "POST",
        headers: {
        "Content-Type": "application/json",
        },
